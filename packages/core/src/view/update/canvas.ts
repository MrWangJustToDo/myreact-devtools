...
export function draw(nodeToData: Map<HTMLElement, Data>): void {
  drawWeb(nodeToData);
}

type DataWithColorAndNode = Data & {
  color: string;
  node: HTMLElement;
};

function iterateNodes(nodeToData: Map<HTMLElement, Data>, execute: (data: DataWithColorAndNode) => void) {
...
function drawLabel(context: CanvasRenderingContext2D, rect: Rect, text: string, color: string): void {
  const { left, top } = rect;
